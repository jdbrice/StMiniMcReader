...
public:
  ...
  virtual ~StMiniMcPair() {}
    ...
    float    commonFrac() const { return static_cast<float>(mNCommonHit)/static_cast<float>(allPts()); }
    float    dominFrac() const { return static_cast<float>(mDominCommonHit)/static_cast<float>(allPts());}
